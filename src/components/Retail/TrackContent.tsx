...
import RateOrderBtn from './Track/RateOrder';
...
export default function TrackContent({data}: any) {
  ...
  useEffect(() => {
    ...
    setTimer({
      ...
      day: Math.abs(moment(expireTime).diff(currentTime, 'days') % 1),
      hour: Math.abs(moment(expireTime).diff(currentTime, 'hours') % 24),
...
const trackStyles = StyleSheet.create({
  ...
  timeUnit: {fontSize: 35, color: '#888'},
  time: {},
  actions: {
    position: 'absolute',
    bottom: 0,
